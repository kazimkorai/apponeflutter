...
class _EditBusinessProfileScreenState extends State<EditBusinessProfile> {
  var radioValue;
  ...
  void populateCountryListForDropdown(List<CountryKazim> countryData) {
    ...
    // selectedCountryHint = countryData[201].name;
  }
    ...
    return Scaffold(
        ...
        body: BaseScrollView().baseView(context, [
          ...
          Padding(
            ...
            child:
            plainTextFieldEmail(Localization.stLocalized('emailUsed'), emailTC),
          ),
          Padding(
            padding: const EdgeInsets.only(top: 30, left: 50, right: 50),
            child: plainTextField(
                  ...
                  child: ButtonTheme(
                    ...
                    child: DropdownButton<String>(
                      ...
                      onChanged: (value) {
                        ...
                        listOfCities.clear();
                        getCitiesByCountryId(model, value.substring(0, value.indexOf('-')));
                        setState(() {
                          selectedCountryHint = value;
                          _valueCountry = value;
                           countryId=_valueCountry.substring(0, value.indexOf('-'));
                          print("**countryId===> "+_valueCountry.substring(0, value.indexOf('-')));
                          _valueCity="Select City";

                        });
                      },
                      value: selectedCountryHint == ""? "":selectedCountryHint,
                    ),
                  ),
                ),
              ),
                          ...
                          child: DropdownButton<String>(
                                ...
                                Expanded(
                                  child: Text(
                                    _valueCity.substring(
                                        _valueCity.indexOf("-") + 1, _valueCity.length),
                                    style: CTheme.textRegular18White(),
                                  ),
                                ),
                              ],
                            ...
                            onChanged: (value) {
                              setState(() {
                                ...
                                _valueCity = value;
                                cityId=_valueCity.substring(0, value.indexOf('-'));
                                print("**cityId==>"+cityId);
                              });
                            },
          ...
          Padding(
            ...
            child: roundedSquareButton(
                Localization.stLocalized('createBusinessProfile'), 50,
                () async {
              if (validateFields()) {
                ...
                CTheme.showCircularProgressDialog(context);
                String createBusinessAccountStatus =
                await createBusinessAccountFirebase(
                    email: emailTC.text.toString());
                if (createBusinessAccountStatus == "Successful") {
                  registerBusinessAccount(userBuisnessData);
                } else {
                  print('error' + createBusinessAccountStatus);
                }
              }
            }),
          )
  ...
  TextField plainTextFieldEmail(String hintText, TextEditingController textEditingController) {
    return TextField(
 ...
 /*   if (imageFile == null) {
    ...
    else if(selectedInterestsList.isEmpty){
      CTheme.showAppAlertOneButton(         "Okay",context: context,title: 'Field Empty',
...
/*    prefCountry_id = prefs.getString(Globals.countryId);
    prefCity_id = prefs.getString(Globals.cityId);*/
    ...
    if (model.intrestsSuccess) {
      setState(() {
        ...
        for (int index = 0; index < model.intrestsData.data.category.length; index++) {
    ...
    try {
      {
        ...
        userImageFile != null
          ...
          signUpSuccess = error
            ...
            : await FirebaseFirestore.instance
            .collection('userBuisnessAccount')
            .doc(uid).collection("BuisnessAccount")
            .add({
          'userId': uid,
          'fcmToken': await getToken(),
          'createdAt': FieldValue.serverTimestamp(),
  ...
  registerBusinessAccount(parameterMap) async {

    CreateBuisnessAccountViewModel model = new CreateBuisnessAccountViewModel();
    await model.createBuisnessAccount(parameterMap);
    if (model.createBuisnessSuccess) {
      Navigator.pop(context);
      print("Successful");
      print(model.createBuisnessData.data.userId);
          CTheme.showAppAlertOneButton(
              "Okay",
                  context: context,
                  title: 'Business Account Registered',
                  bodyText: "Your Business Account Registered Successfully",
                  handler2: (action) => {
                   Navigator.pushReplacementNamed(context, '/profile_home')
                  });
    } else {
      Navigator.pop(context);
      print("Error Create Business Profile");
      CTheme.showAppAlertOneButton(
          "Okay",
          context: context,
          title: 'Error Create Business Profile',
          bodyText: "Error Create Business Profile.");
    }
  }
